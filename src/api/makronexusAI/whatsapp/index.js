...
async function sendWhatsAppMessage(url, headers, phone, name, languageCode = "en_US") {
  const messagePayload = {
    "messaging_product": "whatsapp",
    "to": phone,
    "type": "template",
...
whatsAppRouter.post('/whatsapp-message', async (req, res) => {
  try {
    ...
    await sendWhatsAppMessage(url, headers, phone, "makronexus_intro", "en");
  } catch (error) {
    console.error('Error:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
    ...
    if (isValidWebhookRequest(bodyParam)) {
      ...
      if (messageData) {
        ...
        if (!user) {
          ...
          await sendWhatsAppMessageWithTemplate(url, headers, "+" + from, "call_to_register", "en_US");
        }
